{
  ...
  const TString saveDirectory = "/sbnd/app/users/lynnt/my_larsoft/v09_73_00/opt0_crumbs/nom_OpT0Score";
  using namespace std;
  ...
  leg->AddEntry(hNuSliceOptCrumbsScore, "OpT0 (Score Only) #nu Slices", "l");
  leg->AddEntry(hNotNuSliceOptCrumbsScore, "OpT0 (Score Only) Other Slices", "l");
 ...
 TLine *line_crumbs_2 = new TLine(0.05,0,0.05,1.04*hNotNuSliceNomCrumbsScore->GetMaximum());
  ...
  rocLeg->AddEntry(optcrumbsROC,"OpT0 (Score Only) CRUMBS Score","lp");
  rocLeg->Draw();
  ...
  rocZoomLeg->AddEntry(optcrumbsROC,"OpT0 (Score Only) CRUMBS Score","lp");
  rocZoomLeg->Draw();
